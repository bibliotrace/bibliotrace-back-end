...
describe("DAO testing suite", () => {
  ...
  beforeAll(async () => {
    ...
    dummyAudit = {
      book_id: 10,
      // last_audit_date: new Date().toISOString().slice(0, 19).replace("T", " "),
      state_id: 10,
      expected_amount: 10,
      actual_amount: 10,
    };
  ...
  function parseAutogeneratedFieldsForEquality(response: Response<any>, entity: any) {
  ...
  test("Successful creation of new entities", async () => {
    for (const [entityName, { entity, dao }] of entityDaoMap2) {
      ...
      parseAutogeneratedFieldsForEquality(response, entity);
  ...
  test("Creation of duplicate entities throws an error", async () => {
    for (const [entityName, { entity, dao }] of entityDaoMap) {
      const response = await dao.create(entity);
      expect(response).toBeDefined();
  ...
  test.only("Retrieval of entities by key and value with invalid value is empty", async () => {
    for (const [entityName, { entity, dao }] of entityDaoMap) {
