...
describe("DAO testing suite", () => {
  ...
  beforeAll(async () => {
    ...
    entityDaoMap = new Map([
      ...
      ["Genre", { entity: dummyGenreType, dao: genreDao }],
    ...
    entityDaoMap2 = new Map([
      ...
      ["Genre", { entity: dummyGenreType2, dao: genreDao }],
  ...
  describe("Create tests", () => {
    ...
    test("Failed bookGenre creation with invalid book id", async () => {
      const invalidBookGenre = { ...dummyBookGenre };
      invalidBookGenre.book_id = 100;
      const response = await bookGenreDao.create(invalidBookGenre);
      expectServerErrorResponseOnInvalidCreate(response, "book genre");
    });

    test("Failed bookGenre creation with invalid genre id", async () => {
      const invalidBookGenres = { ...dummyBookGenre2 };
      invalidBookGenres.genre_id = 100;
      const response = await bookGenreDao.create(invalidBookGenres);
      expectServerErrorResponseOnInvalidCreate(response, "book genre");
    });

    test("Failed tag creation with invalid book id", async () => {
      const invalidBookTag = { ...dummyBookTag2 };
      invalidBookTag.book_id = 100;
      const response = await bookTagDao.create(invalidBookTag);
      expectServerErrorResponseOnInvalidCreate(response, "book tag");
    });
  ...
  describe("Get all tests", () => {
    test("Successful retrieval of all entities", async () => {
      ...
      for (const [entityName, { entity, dao }] of entityDaoMap) {
        ...
        expect(response.statusCode).toBe(200);
        expect(response.object).toBeDefined();
        if (dao.tableName === "genre") {
          expect(response.object.length).toBe(2);
          parseArrayForEquality(response, [dummyGenreType, dummyGenreType2]);
        } else {
          expect(response.object.length).toBe(2);
          parseArrayForEquality(response, [entity, entityDaoMap2.get(entityName).entity]);
        }
